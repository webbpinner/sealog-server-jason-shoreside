...
def convertEventRecords(event_records_fn):
    ...
    loweringID = event_records_fn.split("/")[-1].split('_')[0]
    with open(event_records_fn) as event_records_fp:
        ...
        for event in events:
            ...
            if new_event['event_value'] == "SuliusCam":
                new_event['event_options'][0]['event_option_value'] = os.path.join('/', loweringID, 'SuliusCam', 'SuliusCam.' + new_event['event_options'][0]['event_option_value'].split('_')[-1][:8] + "_" + new_event['event_options'][0]['event_option_value'].split('_')[-1][8:14] + '000' + new_event['event_options'][0]['event_option_value'].split('_')[-1][14:])
